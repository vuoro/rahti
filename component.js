...
class Instance {
	run(inputComponent, inputProps) {
		...
		const key = props?.key;
		const instance =
			getInstance(Component, parentId, key) ||
			createInstance(Component, parentId, key);
		const id = instanceIds.get(instance);

		if (parentId !== null)
			currentIndexes.set(parentId, currentIndexes.get(parentId) + 1);
...
const createInstance = (Component, parentId, key) => {
	...
	const instance =
		instancePool.length > 0 ? instancePool.pop() : new Instance();
	instance.id = id;
...
const getInstance = (Component, parentId, key) => {
	...
	if (children) {
		...
		if (
			...
			for (
				let index = currentIndex + 1, { length } = children;
				index < length;
				index++
			) {
...
const run = (id, instance, newArguments, newProps, Component) => {
	...
	try {
		result = Component.apply(
			instance,
			prepareArguments(newProps, newArguments),
		);
...
const runUpdateQueue = async () => {
	for (const id of updateQueue) {
		...
		try {
			newValue = (async ? startAsync : start)(
				id,
				instance,
				undefined,
				undefined,
